...
class CoinbaseIngestor:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.warning(f"Kafka connection failed (attempt {attempt + 1}/{max_retries}): {e}")
    ...
    def _reconnect_kafka(self):
        ...
        delay = self.reconnect_base_delay * (2 ** min(self.reconnect_attempts, 6))  # Cap at 64 seconds
        logger.warning(f"Reconnecting to Kafka (attempt {self.reconnect_attempts + 1}/{self.max_reconnect_attempts}) in {delay}s...")
        time.sleep(delay)
            ...
            while self.running and not self.shutdown_event.is_set():
                ...
                try:
                    ...
                    if not self.running or self.shutdown_event.is_set():
                        break
                    logger.info("WebSocket closed, attempting reconnection in 5 seconds...")
                    time.sleep(5)
                except Exception as e:
                    if self.running and not self.shutdown_event.is_set():
    ...
    def cleanup(self):
        ...
        logger.info(f"Starting graceful cleanup. Total messages processed: {self.message_count}")
