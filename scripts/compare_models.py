...
def print_comparison_table(results: List[Dict]):
    ...
    for r in sorted_results:
        ...
        print(
            f"{r['model_name']:<20} {r['pr_auc']:>10.4f} {r['roc_auc']:>10.4f} {r['f1']:>10.4f} {r['precision']:>10.4f} {r['recall']:>10.4f} {latency_str:>12}"
        )
    ...
    print(
        f"    PR-AUC: {winner['pr_auc']:.4f} | F1: {winner['f1']:.4f} | Recall: {winner['recall']:.4f}"
    )
...
def print_detailed_results(results: List[Dict]):
    ...
    for r in results:
        ...
        print(
            f"    Actual positives:     {r['n_positive']} ({r['n_positive']/r['n_samples']*100:.2f}%)"
        )
        print(
            f"    Predicted positives:  {r['n_predicted_positive']} ({r['n_predicted_positive']/r['n_samples']*100:.2f}%)"
        )
...
def main():
    ...
    parser.add_argument(
        "--target", type=str, default="volatility_spike", help="Target column name"
    )
    ...
    if args.holdout_pct > 0:
        ...
        print(
            f"    Using last {args.holdout_pct*100:.0f}% as holdout: {len(df)} samples"
        )
    ...
    for model_name, model_path in models:
        ...
        try:
            ...
            print(
                f"    [OK] PR-AUC: {metrics['pr_auc']:.4f}, F1: {metrics['f1']:.4f}, Recall: {metrics['recall']:.4f}"
            )
