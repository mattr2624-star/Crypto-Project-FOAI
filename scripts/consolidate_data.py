...
def consolidate_features(
    ...
    for file_path in input_files:
        ...
        try:
            ...
            print(f"✓ Loaded {path.name}: {len(df)} samples, spike_rate: {df['volatility_spike'].mean():.2%}")
            all_dfs.append(df)
    ...
    if remove_duplicates:
        ...
        if min_timestamp_gap_seconds > 0:
            ...
            print(f"Removed {close_removed} rows too close in time (< {min_timestamp_gap_seconds}s)")
    ...
    print(f"Time range: {consolidated['timestamp'].min()} to {consolidated['timestamp'].max()}")
    print(f"Duration: {(consolidated['timestamp'].max() - consolidated['timestamp'].min()).total_seconds() / 3600:.2f} hours")
...
def create_stratified_splits(
    ...
    print(f"\nSplit Statistics:")
    print(f"{'Set':<12} {'Samples':<10} {'Spike Rate':<12} {'Spikes':<10} {'Non-Spikes':<10}")
    print("-" * 60)
    ...
    for name, split_df in [("Train", train_df), ("Validation", val_df), ("Test", test_df)]:
        ...
        print(f"{name:<12} {len(split_df):<10} {spike_rate:<12.2%} {spikes:<10} {non_spikes:<10}")
...
def main():
    ...
    parser.add_argument(
        "--no-dedup",
        action="store_true",
        help="Don't remove duplicate timestamps"
    )
