    ...
    try:
        ...
        for message in consumer:
            ...
            try:
                ...
                logger.warning(f"Skipping non-JSON message at offset {message.offset}: {e}")
        ...
        if valid_count < min_messages:
            logger.warning(f"⚠️  Only consumed {valid_count} valid JSON messages (seen {total_seen} total, expected {min_messages})")
            consumer.close()
            return False
    ...
    try:
        ...
        if partitions:
            ...
            for partition in sorted(partitions):
                ...
                logger.info(f"  Partition {partition}: {messages} messages (offset {beginning} to {end})")
