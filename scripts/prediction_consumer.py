...
def wait_for_api(max_retries: int = 30, retry_interval: int = 2) -> bool:
    ...
    for i in range(max_retries):
        ...
        print(f"[WAIT] Attempt {i+1}/{max_retries} - API not ready, retrying in {retry_interval}s...")
    ...
    for attempt in range(retry_count):
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"[WARN] Prediction failed with status {response.status_code}: {response.text}")
...
def run_demo_mode(interval: float = 2.0, duration: int = 0):
    ...
    while not shutdown_requested:
        ...
        if result:
            ...
            spike_pct = (spike_count / prediction_count * 100) if prediction_count > 0 else 0
            
            status = "🔴 SPIKE" if is_spike else "🟢 Normal"
            print(f"[{datetime.now().strftime('%H:%M:%S')}] #{prediction_count} Score: {score:.4f} {status} | Spike rate: {spike_pct:.1f}%")
        else:
            print(f"[{datetime.now().strftime('%H:%M:%S')}] Prediction failed, retrying...")
        ...
        try:
            for message in consumer:
                ...
                if result:
                    prediction_count += 1
                    score = result.get("scores", [0])[0]
                    print(f"[{datetime.now().strftime('%H:%M:%S')}] #{prediction_count} Score: {score:.4f}")
...
def main():
    parser = argparse.ArgumentParser(description="Prediction Consumer for Dashboard Metrics")
