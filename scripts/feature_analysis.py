...
def load_data(features_path: str) -> Tuple[pd.DataFrame, pd.Series]:
    ...
    exclude_cols = [
        ...
        'price', 'best_bid', 'best_ask', 'spread', 'spread_bps',
        'time_since_last_trade', 'gap_seconds',
        ...
        'price_return_1min', 'price_return_5min', 'price_volatility_5min',
        'bid_ask_spread', 'bid_ask_spread_bps'  # If these are duplicates
    ]
...
def univariate_feature_selection(X: pd.DataFrame, y: pd.Series, 
                                  k: int = 15) -> List[str]:
    """
...
def compare_feature_sets(X: pd.DataFrame, y: pd.Series,
    ...
    print("="*60)
    print(comparison_df[['feature_set', 'n_features', 'mean_pr_auc', 'std_pr_auc']].to_string(index=False))
    
    return comparison_df


def create_visualizations(importance_df: pd.DataFrame,
...
def main():
    ...
    rfe_features, rfe_ranking = recursive_feature_elimination(X, y, cv_folds=args.cv_folds)
    ...
    print(f"Cross-Validated PR-AUC: {best_set['mean_pr_auc']:.4f} ± {best_set['std_pr_auc']:.4f}")
    print(f"\nFeatures:")
    ...
    with open(output_dir / 'recommended_features.txt', 'w') as f:
        ...
        f.write(f"# PR-AUC: {best_set['mean_pr_auc']:.4f} ± {best_set['std_pr_auc']:.4f}\n")
